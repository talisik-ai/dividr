...
export const VideoPreview: React.FC<VideoPreviewProps> = ({ className }) => {
  ...
  const calculateContentScale = useCallback(() => {
    if (!containerSize.width || !containerSize.height)
      return {
        scaleX: 1,
        scaleY: 1,
        actualWidth: preview.canvasWidth,
        actualHeight: preview.canvasHeight,
      };
  ...
  const renderFrame = useCallback(() => {
    ...
    try {
      const { scaleX, scaleY, actualWidth, actualHeight } =
 ...
 */}
      ...
      {tracks.length === 0 && (
        ...
        >
          <div
            className={`text-center p-4 transition-colors duration-200 ${
              dragActive ? 'text-blue-500' : 'text-gray-400 hover:text-gray-300'
            }`}
          >
            ...
            <p className="mx-auto text-sm md:text-md lg:text-lg">
              {dragActive
                ...
                : 'Drop media files here or click to add'}
            </p>
          </div>
        </motion.div>
      )}
