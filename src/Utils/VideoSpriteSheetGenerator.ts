...
export class VideoSpriteSheetGenerator {
  ...
  private static estimateCacheEntrySize(
    ...
    let size = 0;
    for (const sheet of result.spriteSheets) {
      size += sheet.width * sheet.height * 0.1; // Rough bytes estimate for JPEG
    ...
    try {
      ...
      for (let sheetIndex = 0; sheetIndex < numberOfSheets; sheetIndex++) {
        ...
        const spriteSheetCommand = [
          ...
          videoPath,
          '-ss',
          startTime.toFixed(3), // Use higher precision for timing
          '-t',
          sheetDuration.toFixed(3), // Use higher precision for duration
          '-vf',
          [
            `fps=1/${intervalSeconds}`, // Extract frames at interval
            `scale=${thumbWidth}:${thumbHeight}:force_original_aspect_ratio=decrease`, // Maintain aspect ratio
            `pad=${thumbWidth}:${thumbHeight}:(ow-iw)/2:(oh-ih)/2:black`, // Pad with black if needed
        ...
        console.log(
          ...
          `\n   • ${finalFrameCount} planned frames (${thumbnailsInSheet} requested, ${maxPossibleFrames} max possible)`,
          ...
          `\n   • FFmpeg command: tile=${optimalCols}x${optimalRows}`,
  ...
  private static calculateOptimalGrid(frameCount: number): {
    ...
    // For small frame counts, single row is efficient
    if (frameCount <= 20) {
      return { cols: frameCount, rows: 1, emptySlots: 0 };
    }

    // For larger counts, find optimal balance
    let bestGrid = { cols: frameCount, rows: 1, emptySlots: 0 };
    let minScore = Infinity;

    // Try different configurations
    const maxCols = Math.min(frameCount, Math.ceil(Math.sqrt(frameCount * 2))); // Reasonable limit

    for (let cols = 1; cols <= maxCols; cols++) {
      const rows = Math.ceil(frameCount / cols);
      const totalSlots = cols * rows;
      const emptySlots = totalSlots - frameCount;

      // Score based on empty slots and aspect ratio
      const aspectRatio = Math.max(cols / rows, rows / cols);
      const aspectPenalty = aspectRatio > 10 ? (aspectRatio - 10) * 5 : 0;
      const score = emptySlots + aspectPenalty;

      if (score < minScore) {
        minScore = score;
        bestGrid = { cols, rows, emptySlots };
      }

      // Prefer perfect fits with reasonable aspect ratios
      if (emptySlots === 0 && aspectRatio <= 8) {
        return { cols, rows, emptySlots: 0 };
      }
    }
  ...
  private static createCacheKey(options: SpriteSheetOptions): string {
    ...
    return `sprite_${filename}_${roundedStartTime}_${roundedDuration}_${roundedInterval}_${thumbWidth}x${thumbHeight}_q${quality}`;
  }
  ...
  static async generateForTrack(
    ...
    return this.generateSpriteSheets({
      ...
      quality: 3, // Good quality/size balance
    });
  }
}
